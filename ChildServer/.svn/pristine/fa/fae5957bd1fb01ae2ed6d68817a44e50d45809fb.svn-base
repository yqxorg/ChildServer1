...
public class ActiveMqReceiver  implements Runnable {
	...
	{		
		logger.info("mq Received:"+receiveText+";currentThread:"+Thread.currentThread().getId());
		...
		if(receiveText.length()>26){
			...
			logger.info(String.format("mq received sn:%s,cmd:%s,data:%s",snnumber,cmd, receiveText));						
			//添加到重发队列
	...
	private static void SendRepeatByAck(String clientNum, String cmd,String ack,String sendChar)
	  {
		...
		if(bkeyExist == Boolean.TRUE)
		{
			...
			logger.info(String.format("first,no ack:%s,%s,%s", clientNum, cmd, sendChar));  
		}
		else
		{
			logger.info(String.format("first try,ack ok:%s,%s,%s", clientNum, cmd, sendChar));  
			return ;
		}
		...
		if(bkeyExist == Boolean.TRUE)
		{
			//发送
			DiscardServerHandler.SendToClient(clientNum, cmd, sendChar, null,false);	
			logger.info(String.format("second try,no ack:%s,%s,%s", clientNum, cmd, sendChar));  
		}
		else
		{
			logger.info(String.format("second try,ack ok:%s,%s,%s", clientNum, cmd, sendChar));  
			return ;
		}
		try {
			Thread.sleep(3000);
		...
		if(bkeyExist == Boolean.TRUE)
		{
			...
			logger.info(String.format("third try,no ack,record exception:%s,%s,%s", clientNum, cmd, sendChar));  
		}
		else
		{
			logger.info(String.format("third try,ack ok:%s,%s,%s", clientNum, cmd, sendChar));  
		}
